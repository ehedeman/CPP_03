...
/*   Updated: 2024/09/20 13:32:09 by ehedeman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void frag_until_they_die(FragTrap &a, FragTrap &b)
{
	std::cout << a.getName() << " vs " << b.getName() << "!" << std::endl;
	for (int i = 0; i < 5; i++)
	{
...
static void frag_with_repair(FragTrap &c, FragTrap &d)
{
	std::cout << c.getName() << " vs " << d.getName() << "!" << std::endl;
	for (int i = 0; i < 11; i++)
	{
...
{
	std::cout << std::endl;
	FragTrap a("S1");	//a gets constructed with default 0 attack demage		//attack demage is set
	
	FragTrap b(a);			//b is constructed by copying a (to show that)
							//copy constructor and assignment operator work
	FragTrap	c("S3");
	FragTrap	d("S4");
	std::cout << std::endl;
	...
	std::cout << std::endl;
	frag_until_they_die(a, b);
	std::cout << std::endl;
	frag_with_repair(c, d);
}
